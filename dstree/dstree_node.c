...
bool is_leaf(struct dstree_node * node){
    return (node->left_child == NULL && node->right_child == NULL) || node->is_leaf == 'T';
}
				...
				ts_type * timeseries)
{
  ...
  if (node->ts_index == NULL)
  {
    node->ts_index = (unsigned int*) calloc(max_leaf_size, sizeof(unsigned int));
    if (node->ts_index == NULL)
    {
        fprintf(stderr, "Error in dstree_index.c: Could not allocate ts_index array.\n");
        return FAILURE;              
    }
  }
  node->ts_index[node->node_size - 1] = index->first_node->node_size;

//   printf("%d-%d: to node %s %f [", index->first_node->node_size, node->node_size, node->filename, timeseries[0]);
//   for (int i = 0; i < max_leaf_size; ++i) printf("%d ", node->ts_index[i]);
//   printf("]\n");
				...
				ts_type * timeseries)
{
  ...
  if (node->ts_index == NULL)
  {
    node->ts_index = calloc(max_leaf_size, sizeof(unsigned int));
    if (node->ts_index == NULL)
    {
        fprintf(stderr, "Error in dstree_index.c: Could not allocate ts_index array.\n");
        return FAILURE;              
  ...
  unsigned int children_size = node->parent->left_child->node_size + node->parent->right_child->node_size;
  node->ts_index[node->node_size - 1] = node->parent->ts_index[children_size - 1];
  if (children_size >= max_leaf_size){
    if (node->parent->ts_index != NULL){
        free(node->parent->ts_index);
        node->parent->ts_index = NULL;
    }
  }
//   printf("%d-%d: parent node %s to child node %s %f [", index->first_node->node_size, node->node_size, node->parent->filename, node->filename, timeseries[0]);
//   for (int i = 0; i < max_leaf_size; ++i) printf("%d ", node->ts_index[i]);
//   printf("]\n");
